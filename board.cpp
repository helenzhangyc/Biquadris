...
void Board::setUp(string filename1, string filename2, int level1, int level2) {
  if (level1 == 0) {
    ...
    player1Lvl = make_shared<Level5>();
    player2Lvl = make_shared<Level5>();
  }
...
void Board::setLevel(int n, bool b, string file) {
  if (n == 0) {
    ...
    if (b) {
      shared_ptr<Level> l = make_shared<Level5>();
      swap(player1Lvl, l);
    }
    else {
      shared_ptr<Level> l = make_shared<Level5>();
...
bool Board::drop(bool check) {
  ...
  if (lvl->getLevel() == 4 || lvl->getLevel() == 5) {
  p->blockNum++;
  }
  return true;
}
    ...
    if (player->blockNum % 5 == 0) {
      if (player->blockClear == false) {
        ...
        if (needCancel == true) {
          ...
          // move down each row
          for (int x = 0; x <= 10; ++x) {
            player->gameBoard[starpos][x]->clear(); 
          }
...
bool Board::changeBlock(string bType, bool player) {
  ...
  vector<vector<int>>& coordsNew = newBlock->getCoordsCpy();
  int minXnew = 10;
  int minYnew = 0;
  int maxXnew = 0;
  int maxYnew = -17;
  ...
  //update new block's coords to coordsCpy
  if (valid == true) {
...
void Board::changeRandom (bool check, string filename) {
  ...
  if (l->getLevel() == 3 || l->getLevel() == 4 || l->getLevel() == 5) {
...
void Board::scoreDeduction(bool check) {
  if (check == true) {
    player1->score = player1->score - 5;
    w -> setScore(check, player1->score);
  } else {
    player2->score = player2->score - 5;
    w -> setScore(check, player2->score);
  } 
}